...
class MessagesController extends Controller
{
    ...
    public function idFetchData(Request $request)
    {
        ...
        if ($request['type'] == 'user') {
            $fetch = Teacher::where('id', $request['id'])->first();
            if($fetch){
    ...
    public function getFavorites(Request $request)
    {
        ...
        foreach ($favorites as $favorite) {
            $favorite->user = Teacher::where('id', $favorite->favorite_id)->first();
        }
        return Response::json([
    ...
    public function search(Request $request)
    {
        ...
        $records = Teacher::where('id','!=',Auth::user()->id)
    ...
    public function updateSettings(Request $request)
    {
        ...
        if ($request['dark_mode']) {
            $request['dark_mode'] == "dark"
                ? Teacher::where('id', Auth::user()->id)->update(['dark_mode' => 1])  // Make Dark
                : Teacher::where('id', Auth::user()->id)->update(['dark_mode' => 0]); // Make Light
        }
        ...
        if ($request->hasFile('avatar')) {
            ...
            if ($file->getSize() < Chatify::getMaxUploadSize()) {
                if (in_array($file->getClientOriginalExtension(), $allowed_images)) {
                    ...
                    $update = Teacher::where('id', Auth::user()->id)->update(['avatar' => $avatar]);
    ...
    public function setActiveStatus(Request $request)
    {
        $update = $request['status'] > 0
            ? Teacher::where('id', $request['user_id'])->update(['active_status' => 1])
            : Teacher::where('id', $request['user_id'])->update(['active_status' => 0]);
        // send the response
        return Response::json([
            'status' => $update,
        ], 200);
